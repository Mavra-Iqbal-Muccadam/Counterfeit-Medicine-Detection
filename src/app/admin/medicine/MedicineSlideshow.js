...
const MedicineSlideshow = ({
  ...
  const handleViewDetails = async (medicineId) => {
    ...
    if (selected) {
      ...
      if (
        activeSection === "pendingMedicines" &&
        !authenticityResults[medicineId]
      ) {
  ...
  const confirmRejection = async () => {
    try {
      ...
      const medicine = filteredMedicines.find(
        (m) => m.tokenId === medicineToReject,
      );
  ...
  const getManufacturerName = (walletAddress) => {
    if (!walletAddress || !manufacturerList.length)
            ...
            <Box sx={{ mt: 2 }}>
              ...
              {authenticityResults[selectedMedicine.tokenId] && (
                <Box
                  sx={{
                    ...
                    p: 2,
                    borderRadius: 1,
                    backgroundColor: authenticityResults[
                ...
                >
                  ...
                  <Typography
                    variant="body2"
                    sx={{
                      color: authenticityResults[selectedMedicine.tokenId]
                        .isAuthentic
                        ? "#4CAF50"
                        : "#F44336",
                      fontWeight: "medium",
              ...
              >
                ...
                {Object.entries(selectedMedicine).map(([key, value]) => {
                  if (
                    ["tokenId", "walletAddress", "uploadedFiles"].includes(key)
                  )
                    return null;
              ...
              {selectedMedicine.uploadedFiles?.some((f) =>
                f.fileName?.toLowerCase().endsWith(".pdf"),
              ) && (
                <Box sx={{ mt: 3 }}>
                  <Typography
                    variant="subtitle2"
                    sx={{ fontWeight: "bold", color: "#002F6C", mb: 1 }}
                  >
                    Certificate PDF
                  </Typography>
                  {selectedMedicine.uploadedFiles.map((file, index) =>
              ...
              {selectedMedicine.status === "Rejected" &&
                rejectionComments[selectedMedicine.tokenId] && (
                  <Box
                    sx={{
                      mb: 3,
                      p: 2,
                  ...
                  >
                    <Typography
                      variant="subtitle2"
                      sx={{ fontWeight: "bold", color: "#F44336", mb: 1 }}
                    >
                      Rejection Reason
                    </Typography>
                    {rejectionComments[selectedMedicine.tokenId].map(
                      (comment, index) => (
                        <Typography
                          key={index}
                          variant="body2"
                ...
                selectedMedicine.status === "Pending" && (
                  ...
                  >
                    <Button
                      variant="contained"
                      onClick={() =>
                        handleStatusUpdate(selectedMedicine.tokenId, "Accepted")
                      }
                    ...
                    <Button
                      variant="contained"
                      onClick={() =>
                        handleRejectClick(selectedMedicine.tokenId)
                      }
