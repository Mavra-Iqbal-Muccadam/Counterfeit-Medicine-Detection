...
const ManufacturerDashboard = () => {
  ...
  const fetchAllMedicines = async () => {
    if (!walletStatus.address || !walletStatus.approved) {
      setErrorMsg({
        open: true,
        message: "Wallet not connected or not approved",
      });
      return;
    }
    ...
    try {
      ...
      const { pending, accepted, rejected } =
        await fetchMedicinesByManufacturerAndStatus(walletStatus.address);
  ...
  const fetchAcceptedMedicines = async () => {
    ...
    try {
      ...
      setErrorMsg({
        open: true,
        message: "Failed to fetch accepted medicines",
      });
    } finally {
  ...
  const fetchRejectedMedicines = async () => {
    ...
    try {
      ...
      setErrorMsg({
        open: true,
        message: "Failed to fetch rejected medicines",
      });
    } finally {
  ...
  const handleAddMedicine = async (medicineData) => {
    try {
      ...
      setSuccessMsg({
        open: true,
        message: "✅ Medicine registration submitted successfully!",
      });
      handleCloseModal();
      ...
      setErrorMsg({
        open: true,
        message: error.message || "❌ Failed to register medicine",
      });
    } finally {
    ...
    if (selectedMedicine && walletStatus.address) {
      try {
        ...
        const newSaleItem = await addMedicineToSale({
          ...
          excipients:
            selectedMedicine.excipients || selectedMedicine.ingredients || [],
        });
        ...
        sx={{
          filter:
            !walletStatus.loading && !walletStatus.approved
              ...
              : "none",
          pointerEvents:
            !walletStatus.loading && !walletStatus.approved ? "none" : "auto",
          width: "100%",
          height: "100%",
          ...
          {walletStatus.connected ? (
            <>
              ...
              <Typography variant="body1" sx={{ maxWidth: "600px", mb: 4 }}>
                The connected wallet is not registered as an approved
                manufacturer. Please contact the administrator if you believe
                this is an error.
              </Typography>
            </>
          ) : (
            <>
              ...
              <Typography variant="body1" sx={{ maxWidth: "600px", mb: 4 }}>
                Please connect an approved manufacturer wallet to access this
                dashboard.
              </Typography>
            </>
          )}
